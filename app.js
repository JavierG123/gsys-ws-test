...
async function handleDTMF(ws, msg) {
  ...
  if (dtmf === '1') {
    sendAudio(ws, 'TEST_file_example_WAV_5MG.wav');
  }
  if (dtmf === '2') {
...
function sendAudio(ws, audioFilePath) {
  ...
  const MAX_MESSAGE_SIZE = 64000; // Tamaño máximo por mensaje (64 KB)
  const MAX_BURST = 25; // Máximo de mensajes por ráfaga
  const BURST_INTERVAL = 100; // Intervalo entre ráfagas (en milisegundos)
  ...
  fs.readFile(audioFilePath, (err, data) => {
    ...
    const audioBuffer = new Uint8Array(data); // Convertir a Uint8Array
    const totalChunks = Math.ceil(audioBuffer.length / MAX_MESSAGE_SIZE); // Calcular fragmentos totales
    let currentChunk = 0; // Índice del fragmento actual
    let burstCount = 0; // Contador de mensajes en la ráfaga actual

    const sendNextChunk = () => {
      while (burstCount < MAX_BURST && currentChunk < totalChunks) {
        const start = currentChunk * MAX_MESSAGE_SIZE;
        const end = Math.min(start + MAX_MESSAGE_SIZE, audioBuffer.length);
        const chunk = audioBuffer.slice(start, end); // Extraer el fragmento
      ...
      // Si hay más fragmentos, programa la siguiente ráfaga
      if (currentChunk < totalChunks) {
        burstCount = 0; // Reinicia el contador de ráfagas
        setTimeout(sendNextChunk, BURST_INTERVAL);
      } else {
        logMessage(`Archivo de audio enviado: ${audioFilePath}`);
      }
    };

    // Inicia el envío de fragmentos
    sendNextChunk();
  });
}
